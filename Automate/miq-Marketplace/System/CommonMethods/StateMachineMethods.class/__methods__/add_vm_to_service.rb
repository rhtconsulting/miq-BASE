...
# License: GPL v3
#
# Description: This method adds VMs to a service after Post Provisioning as well as set VM group ownership
#
...
begin
  ...
  log_and_update_message(:info, "WS Values: #{ws_values.inspect}")

  prov_tags = @task.get_tags
  log_and_update_message(:info, "Inspecting miq_provision tags: #{prov_tags.inspect}")
  ...
  if service_guid.nil? && service_resource_id.nil?
    unless ws_values[:service_id].blank?
      ...
      log_and_update_message(:info, "Found ws_values[:service_id]: #{service_id.inspect}") unless service_id.nil?
    end
    ...
    unless service_id.nil?
      ...
      unless parent_service.nil?
        ...
        log_and_update_message(:info, "Adding VM: #{vm.name} to Service: #{parent_service.name}", true)
        vm.add_to_service(parent_service)
        log_and_update_message(:info, "Service: #{parent_service.name} vms: #{parent_service.vms.count} tags: #{parent_service.tags.inspect}")
      end
  ...
  unless group_id.nil?
    log_and_update_message(:info, "Found group_id: #{group_id.inspect}") unless group_id.nil?
    # Look up the group by id
    group = $evm.vmdb(:miq_group).find_by_id(group_id)
    unless group.nil?
      log_and_update_message(:info, "Assigning ownership for group: #{group.description} to VM: #{vm.name}", true)
      vm.group = group
    end
  end
  ...
  if ws_values[:flex_reason]
    ...
    log_and_update_message(:info, "Found flex parent_vm: #{parent_vm.name}")

    # Add custom attributes on the provisioned VM
    log_and_update_message(:info, "Adding custom attribute {:flex_reason => #{ws_values[:flex_reason].to_s}} to VM: #{vm.name}", true)
    vm.custom_set(:flex_reason, ws_values[:flex_reason].to_s)
    log_and_update_message(:info, "Adding custom attribute {:flex_vm_name => #{ws_values[:flex_vm_name].to_s}} to VM: #{vm.name}", true)
    vm.custom_set(:flex_vm_name, ws_values[:flex_vm_name].to_s)
    log_and_update_message(:info, "Adding custom attribute {:flex_vm_guid => #{ws_values[:flex_vm_guid].to_s}} to VM: #{vm.name}", true)
    ...
    unless flex_pending.zero?
      ...
      unless parent_vm.tagged_with?('flex_pending', new_flex_pending)
        ...
        log_and_update_message(:info, "Assigning tag: {#{flex_pending} => #{new_flex_pending}} to parent_vm: #{parent_vm.name}", true)
    ...
    unless parent_vm.tagged_with?('flex_current', new_flex_current)
      ...
      log_and_update_message(:info, "Assigning tag: {:flex_current => #{new_flex_current}} to parent_vm: #{parent_vm.name}", true)
